...
export const HeaderBase = ({ t, lng }) => {
        ...
        <MenuItem key={l} className="hover:bg-zinc-100 dark:hover:bg-white/10 rounded-lg">
          {() => (
            <Link
              href={`/${l}`}
              className={
                'font-medium block px-4 py-2 text-sm hover:text-[var(--accent-color)] transition-colors duration-200'
              }
            >
              {l === 'en'
                ? 'English'
  ...
  const navigation = [
    { name: t('projects'), href: `/${lng}/projects` },
    { name: t('about'), href: `/${lng}/about` },
    { name: t('resume'), href: `/${lng}/resume` },
                      ...
                      <div className="flex space-x-1 items-center">
                        {navigation.map((item) => (
                          <Link
                            ...
                            href={item.href}
                            className={
                              'hover:bg-zinc-100 dark:hover:bg-white/10 hover:text-[var(--accent-color)] rounded-full px-3 py-2 text-sm font-medium transition-[background-color] duration-200'
                            }
                          >
                            {item.name}
                          </Link>
              ...
              <div className="space-y-1 px-2 pb-3 pt-2">
                {navigation.map((item) => (
                  <DisclosureButton
                    ...
                    href={item.href}
                    className={
                      'hover:bg-zinc-100 dark:hover:bg-white/10 hover:text-[var(--accent-color)] block rounded-xl px-3 py-2 text-base font-medium transition-[background-color] duration-200'
                    }
